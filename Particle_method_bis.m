...
alpha=0.7; %Constraint on the flux at the AV position. (f(t,0+) \leq 1/4 alpha)
...
for n=1:M
    ...
    for k=1:N
     ...
     if v(U_discr(n,end-k))<=v(U_discr(n,end-k+1))
      ...
      if (-eps < X_discr(n+1,end-k+1)) &&  (X_discr(n+1,end-k+1)<eps)
        %  d=((X_discr(1,end-k+1)-X_discr(1,end-k))*U_discr(1,end-k))/(U_discr(n+1,end-k+1));
       d=((X_discr(1,end-k+1)-X_discr(1,end-k))*U_discr(1,end-k))/(1-alpha*(1-U_discr(n+1,end-k+1)));
       %d=((X_discr(1,end-k+1)-X_discr(1,end-k))*U_discr(1,end-k))*v(U_discr(n+1,end-k))*vaphi(X_discr(n+1,end-k),eps,alpha)/alpha;
       if X_discr(n,end-k)+Delta_t*v(U_discr(n,end-k))*vaphi(X_discr(n,end-k),eps,alpha)< X_discr(n+1,end-k+1)-d
          X_discr(n+1,end-k)=X_discr(n,end-k)+Delta_t*v(U_discr(n,end-k))*vaphi(X_discr(n,end-k),eps,alpha);
          U_discr(n+1,end-k)=((X_discr(1,end-k+1)-X_discr(1,end-k))*U_discr(1,end-k))/(X_discr(n+1,end-k+1)-X_discr(n+1,end-k));
       else
        X_discr(n+1,end-k)=X_discr(n+1,end-k+1)-d;
        %U_discr(n+1,end-k)=1-alpha*(1-U_discr(n+1,end-k+1));
...
hold on
plot(eps*ones(1,length(0:0.1:T)),0:0.1:T,'r')
hold on 
plot(-eps*ones(1,length(0:0.1:T)),0:0.1:T,'r')
